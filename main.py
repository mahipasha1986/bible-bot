...
WRITER_URL = "https://script.google.com/macros/s/AKfycbxetgYAzMs0M_mv_eu0DZLUpxrtwPYDhTs7jHOrcFO4mThc_nl2YmHDvx0JmP7QwiHp/exec"
...
def categories(chat_id):
    buttons = [[{"text": c["button"], "callback_data": f"cat|{c['value']}"}] for c in SONG_CATEGORIES]
    send_msg(chat_id, "🎵 مناسبت مورد نظر را انتخاب کنید:", {"inline_keyboard": buttons})


def random_song(chat_id):
    songs = [r for r in sheet("Songs", use_cache=False) if value(r, "اسم سرود") and value(r, "فایل")]
    if not songs:
...
def random_prayer(chat_id):
    ...
    r = random.choice(rows)
    send_msg(chat_id,
             f"🙏 برای یکی از عزیزان دعا کنیم:\n\n✨ {value(r, 'متن دعا')}\n\n🤍 خداوند برکت و آرامی خود را جاری کند.",
             {"inline_keyboard": [[{"text": "🙌 من هم دعا می‌کنم", "callback_data": "praydone"}]]})
    ...
    if "callback_query" in data:
        ...
        if cb.startswith("book|"):
            ...
            songs = [
                ...
                if norm(value(r, "مناسبت")) == norm(cat) and value(r, "اسم سرود") and value(r, "فایل")
            ]
            ...
            buttons = [[{"text": "🎵 " + value(s, "اسم سرود"), "callback_data": f"catsong|{cat}|{value(s, 'اسم سرود')}"}] for s in songs]
            ...
            send_msg(chat_id, "🙏 ممنون از همراهی شما.\nخداوند برکتتان دهد 🤍")
