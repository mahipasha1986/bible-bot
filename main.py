...
def api_word():
    ...
    suggestions = []
    for w in partial[:20]:
        suggestions.append(value(w, "کلمه"))
...
<div id="songsPage" class="tab-page active">
  <div class="card">
    ...
    <p>به بخش سرودهای ربات «کلمه‌یاب و سرودیاب» خوش آمدید.<br>از گزینه‌های زیر می‌توانید سرودهای مناسبتی یا لیست کامل سرودها را انتخاب کنید.</p>
  ...
  <div class="card">
    <input id="songSearch" placeholder="🔍 جستجوی نام سرود..." oninput="filterSongs()">
...
<div id="wordsPage" class="tab-page">
  <div class="card">
    ...
    <input id="wordSearch" placeholder="مثال: ابا" onkeydown="if(event.key==='Enter') searchWord()">
    <button class="purple" onclick="searchWord()">🔍 جستجو</button>
    <div id="wordStatus" class="small"></div>
    <div id="wordButtons" class="word-buttons">
      <button class="green" onclick="showWordPart('meaning')">📖 معنی</button>
      <button class="gold" onclick="showWordPart('root')">💡 ریشه</button>
      <button class="blue" onclick="showWordPart('verse')">📜 آیه مرتبط</button>
    </div>
...
<div class="bottom-nav">
  <div id="navSongs" class="nav-item active" onclick="showTab('songs')"><span class="nav-icon">🎼</span>سرودها</div>
  <div id="navBooks" class="nav-item" onclick="showTab('books')"><span class="nav-icon">📚</span>کتابخانه</div>
  <div id="navWords" class="nav-item" onclick="showTab('words')"><span class="nav-icon">📖</span>کلمات</div>
</div>
...
async function loadSongs(){currentSource="songs";closePlayer();setSongStatus("⏳ در حال دریافت لیست سرودها...");const res=await fetch("/api/songs");const data=await res.json();allSongs=data.songs||[];currentPage=0;songSearch.value="";renderSongPage();setSongStatus("🎼 لیست کامل سرودها");}
async function loadCategory(index){currentSource="category";closePlayer();setSongStatus("⏳ در حال دریافت سرودهای مناسبتی...");const res=await fetch("/api/category/"+index);const data=await res.json();allSongs=data.songs||[];currentPage=0;songSearch.value="";renderSongPage();setSongStatus(data.title||"🎵 سرودهای مناسبتی");}
...
function renderSongs(songs,showPagination){songsDiv=document.getElementById("songs");songsDiv.innerHTML=""; if(!songs.length){songsDiv.innerHTML="<div class='small'>موردی پیدا نشد.</div>";return;} songs.forEach(song=>{const div=document.createElement("div");div.className="item";div.innerText="🎵 "+song.name;div.onclick=()=>openPlayer(song);songsDiv.appendChild(div);}); if(showPagination) renderSongPagination(songsDiv);}
function renderSongPagination(container){const totalPages=Math.ceil(allSongs.length/songsPerPage); if(totalPages<=1)return; const pageInfo=document.createElement("div");pageInfo.className="small";pageInfo.innerText="صفحه "+(currentPage+1)+" از "+totalPages;container.appendChild(pageInfo); const nav=document.createElement("div");nav.style.marginTop="12px";nav.style.display="flex";nav.style.gap="10px"; const prev=document.createElement("button");prev.innerText="⬅️ صفحه قبل";prev.className="blue";prev.style.flex="1";prev.disabled=currentPage===0;prev.style.opacity=currentPage===0?"0.45":"1";prev.onclick=()=>{if(currentPage>0){currentPage--;closePlayer();renderSongPage();window.scrollTo({top:0,behavior:"smooth"});}}; const next=document.createElement("button");next.innerText="صفحه بعد ➡️";next.className="purple";next.style.flex="1";next.disabled=currentPage>=totalPages-1;next.style.opacity=currentPage>=totalPages-1?"0.45":"1";next.onclick=()=>{if(currentPage<totalPages-1){currentPage++;closePlayer();renderSongPage();window.scrollTo({top:0,behavior:"smooth"});}}; nav.appendChild(prev);nav.appendChild(next);container.appendChild(nav);}
function filterSongs(){const q=songSearch.value.toLowerCase().trim(); if(!q){renderSongPage();return;} const filtered=allSongs.filter(s=>(s.name||"").toLowerCase().includes(q));renderSongs(filtered.slice(0,100),false);setSongStatus(filtered.length>100?"🔍 بیش از ۱۰۰ نتیجه پیدا شد؛ لطفاً دقیق‌تر جستجو کنید.":"🔍 نتیجه جستجو: "+filtered.length+" مورد");}
function openPlayer(song){selectedSong=song;playerTitle.innerText="🎵 "+song.name;audioPlayer.src="/api/audio?source="+currentSource+"&index="+song.index;playerCard.style.display="block";setSongStatus("🎧 برای پخش، دکمه Play را بزنید.");playerCard.scrollIntoView({behavior:"smooth",block:"start"});}
...
async function sendSelectedSong(){if(!selectedSong){setSongStatus("ابتدا یک سرود را انتخاب کنید.");return;} await sendSong(selectedSong.index);}
async function sendSong(index){const chatId=getChatId(); if(!chatId){alert("لطفاً این صفحه را از داخل تلگرام باز کنید.");return;} setSongStatus("⏳ در حال ارسال سرود...");const res=await fetch("/api/send_song",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:chatId,index:index,source:currentSource})});const data=await res.json();setSongStatus(data.ok?"✅ سرود در تلگرام برای شما ارسال شد.":"❌ ارسال سرود انجام نشد.");}
async function loadBooks(){bookStatus.innerText="⏳ در حال دریافت کتاب‌ها...";const res=await fetch("/api/books");const data=await res.json();allBooks=data.books||[];bookSearch.value="";renderBooks(allBooks);bookStatus.innerText="📚 کتابخانه";}
function renderBooks(books){booksDiv=document.getElementById("books");booksDiv.innerHTML=""; if(!books.length){booksDiv.innerHTML="<div class='small'>کتابی پیدا نشد.</div>";return;} books.forEach(book=>{const div=document.createElement("div");div.className="item";div.innerText="📖 "+book.name;div.onclick=()=>sendBook(book.index);booksDiv.appendChild(div);});}
...
async function sendBook(index){const chatId=getChatId(); if(!chatId){alert("لطفاً این صفحه را از داخل تلگرام باز کنید.");return;} bookStatus.innerText="⏳ در حال ارسال کتاب...";const res=await fetch("/api/send_book",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:chatId,index:index})});const data=await res.json();bookStatus.innerText=data.ok?"✅ کتاب در تلگرام برای شما ارسال شد.":"❌ ارسال کتاب انجام نشد.";}
async function searchWord(){const q=wordSearch.value.trim(); if(!q){wordStatus.innerText="لطفاً یک کلمه وارد کنید.";return;} wordStatus.innerText="⏳ در حال جستجو...";wordResult.style.display="none";wordButtons.style.display="none";wordSuggestions.innerHTML=""; const res=await fetch("/api/word?q="+encodeURIComponent(q)); const data=await res.json(); if(data.found){currentWordData=data;wordStatus.innerText="🔍 اطلاعات «"+data.word+"» یافت شد.";wordButtons.style.display="flex";wordButtons.style.flexDirection="column";}else{currentWordData=null;wordStatus.innerText="مورد دقیق پیدا نشد."; if(data.suggestions&&data.suggestions.length){wordSuggestions.innerHTML="<div class='small'>موارد نزدیک:</div>";data.suggestions.forEach(w=>{const div=document.createElement("div");div.className="item";div.innerText="🔍 "+w;div.onclick=()=>{wordSearch.value=w;searchWord();};wordSuggestions.appendChild(div);});}}}
function showWordPart(part){if(!currentWordData)return; wordResult.style.display="block"; if(part==="meaning") wordResult.innerText="📖 معنی:\n\n"+(currentWordData.meaning||"ثبت نشده است."); if(part==="root") wordResult.innerText=(currentWordData.root_title||"💡 ریشه")+":\n\n"+(currentWordData.root||"ثبت نشده است."); if(part==="verse") wordResult.innerText="📜 آیه مرتبط:\n\n"+(currentWordData.verse||"ثبت نشده است."); wordResult.scrollIntoView({behavior:"smooth",block:"start"});}
</script>
</body>
</html>
"""
