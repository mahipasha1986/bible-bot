...
def songs_menu(chat_id):
    send_msg(
        chat_id,
        "🎼 برای ورود به بخش سرودها روی دکمه زیر بزنید:",
        {"inline_keyboard": [[
            {"text": "🎼 ورود به بخش سرودها", "web_app": {"url": WEBAPP_URL}}
        ]]}
    )
...
def api_word():
    ...
    if not q:
        return jsonify({"ok": False, "found": False, "suggestions": []})
...
<div id="songsPage" class="page-section active">
<div class="card">
  <h1>🎼 سرودهای پرستشی</h1>
  <p>
    به بخش سرودهای ربات «کلمه‌یاب و سرودیاب» خوش آمدید.<br>
    از گزینه‌های زیر می‌توانید سرودهای مناسبتی، لیست کامل سرودها یا «یک سرود برام انتخاب کن» را انتخاب کنید.
  </p>
  ...
  <button class="purple" onclick="randomSong()">🎵 یک سرود برام انتخاب کن</button>
</div>
...
<div id="booksPage" class="page-section">
  <div class="card">
    <h1>📚 کتابخانه</h1>
    <p>کتاب‌های موجود را مشاهده کنید و برای دریافت فایل روی نام کتاب بزنید.</p>
...
<div id="wordsPage" class="page-section">
  <div class="card">
    ...
    <div id="wordResult" class="word-result-box"></div>
    <div id="wordSuggestions"></div>
  </div>
</div>

<div class="bottom-nav">
  <button id="tabSongs" class="active" onclick="showPage('songs')">🎼<br>سرودها</button>
  <button id="tabBooks" onclick="showPage('books')">📚<br>کتابخانه</button>
  <button id="tabWords" onclick="showPage('words')">📖<br>کلمات</button>
</div>
...
function showPage(page){
  document.getElementById("songsPage").classList.remove("active");
  document.getElementById("booksPage").classList.remove("active");
  document.getElementById("wordsPage").classList.remove("active");

  document.getElementById("tabSongs").classList.remove("active");
  document.getElementById("tabBooks").classList.remove("active");
  document.getElementById("tabWords").classList.remove("active");

  if(page === "songs"){
    document.getElementById("songsPage").classList.add("active");
  ...
  if(page === "words"){
    ...
    document.getElementById("tabWords").classList.add("active");
  }
...
async function randomSong(){
  ...
  closePlayer();
  setStatus("⏳ در حال انتخاب سرود...");

  const res = await fetch("/api/random_song", {
    method:"POST",
    headers:{"Content-Type":"application/json"},
...
async function searchWord(){
  ...
  if(!q){
    document.getElementById("wordStatus").innerText = "لطفاً یک کلمه وارد کنید.";
    return;
  }

  document.getElementById("wordStatus").innerText = "⏳ در حال جستجو...";
  ...
  if(data.found){
    ...
    document.getElementById("wordStatus").innerText = "🔍 اطلاعات «" + data.word + "» یافت شد.";
    document.getElementById("wordButtons").style.display = "block";
  }else{
    currentWordData = null;

    document.getElementById("wordStatus").innerText = "مورد دقیق پیدا نشد.";

    const sug = document.getElementById("wordSuggestions");
    ...
    if(data.suggestions && data.suggestions.length){
      ...
      data.suggestions.forEach(w => {
        ...
        div.innerText = "🔍 " + w;
        div.onclick = () => {
          document.getElementById("wordSearch").value = w;
          searchWord();
        };
        sug.appendChild(div);
      });
...
function showWordPart(part){
  ...
  if(part === "root"){
    box.innerText = (currentWordData.root_title || "💡 ریشه") + ":\n\n" + (currentWordData.root || "ثبت نشده است.");
  }

  if(part === "verse"){
    box.innerText = "📜 آیه مرتبط:\n\n" + (currentWordData.verse || "ثبت نشده است.");
  }
