...
def send_welcome(chat_id):
    text = (
        ...
        "📖 این ربات برای بنای روحانی شما طراحی شده تا با جستجوی کلمات، "
        "به عمق کلام خدا نزدیک‌تر شوید.\n\n"
        "⚠️ در برخی مواقع ممکن است اولین پاسخ چند لحظه زمان ببرد. "
        "سپاس از شکیبایی شما 🙏"
    )
...
def send_guide(chat_id):
    text = (
        ...
        "🌱 بذر کلام:\n"
        "نام یک کلمه (مثلاً: ابا) را ارسال کنید تا ریشه و معنای آن در کتاب‌مقدس برایتان آشکار شود.\n\n"
        "🎶 پرستش:\n"
        "برای یافتن سرود، کلمه «سرود» را قبل از نام آن بنویسید.\n\n"
        "🙌 اتحاد در دعا:\n"
        "«بارهای یکدیگر را حمل کنید...» (غلاطیان ۶:۲)\n"
        "درخواست‌های دعای خود را با پیشوند «دعا:» ارسال کنید.\n\n"
        "⚠️ نکته:\n"
        "در برخی مواقع، اولین پاسخ ممکن است چند لحظه زمان ببرد. سپاس از همراهی شما 🌿"
    )
...
def handle_file(message):
    ...
    if "document" in message:
        ...
        send_message(chat_id, f"📄 کد فایل دریافت شد:\n\nfile_id:\n{file_id}\n\nنام فایل:\n{file_name}")
        return True

    if "audio" in message:
        file_id = message["audio"]["file_id"]
        file_name = message["audio"].get("file_name", "بدون نام")
        send_message(chat_id, f"🎵 کد فایل صوتی دریافت شد:\n\nfile_id:\n{file_id}\n\nنام فایل:\n{file_name}")
        return True

    if "voice" in message:
        file_id = message["voice"]["file_id"]
        send_message(chat_id, f"🎙 کد ویس دریافت شد:\n\nfile_id:\n{file_id}")
        return True
...
def webhook():
    ...
    if "message" in data:
        ...
        if "راهنمای ربات" in text:
            send_guide(chat_id)
            return "ok"

        if "کانال تلگرام" in text:
