...
def songs_menu(chat_id):
    send_msg(
        chat_id,
        "🎼 برای ورود به بخش سرودها و کتابخانه روی دکمه زیر بزنید:",
        {"inline_keyboard": [[
            {"text": "🎼 ورود به سرودها و کتابخانه", "web_app": {"url": WEBAPP_URL}}
        ]]}
    )
...
def api_word():
    ...
    if not q:
        return jsonify({
            "ok": True,
            "found": False,
            "message": "لطفاً یک کلمه وارد کنید.",
            "suggestions": []
        })
    ...
    suggestions = []
    for w in partial[:10]:
        word_name = value(w, "کلمه")
        if word_name:
            suggestions.append(word_name)

    return jsonify({
        "ok": True,
        "found": False,
        "message": "🔍 این مورد یافت نشد، خادمین در حال گسترش آرشیو هستند.",
...
window.searchWord = async function(){
  ...
  try{
    ...
    document.getElementById("wordStatus").innerText = data.message || "🔍 این مورد یافت نشد، خادمین در حال گسترش آرشیو هستند.";
    ...
    document.getElementById("wordStatus").innerText = "❌ خطا در جستجو. لطفاً دوباره تلاش کنید.";
  }
}

window.showWordPart = function(part){
...
<div class="tab-buttons">
  ...
  <button class="blue" onclick="showSection('wordsSection')">📖 کلمات</button>
</div>
...
<div class="card">
  ...
  <p>
    نام کلمه، مکان یا شخصیت کتاب‌مقدس را جستجو کنید.
  </p>

  <input id="wordSearch" placeholder="مثال: ابا" onkeydown="if(event.key==='Enter') window.searchWord()">
  <button class="purple" onclick="window.searchWord()">🔍 جستجو</button>

  <div id="wordStatus" class="small"></div>

  <div id="wordButtons" class="word-buttons">
    <button class="green" onclick="window.showWordPart('meaning')">📖 معنی</button>
    <button class="gold" onclick="window.showWordPart('root')">💡 ریشه</button>
    <button class="blue" onclick="window.showWordPart('verse')">📜 آیه مرتبط</button>
  </div>
...
function showSection(sectionId){
  ...
  document.getElementById("wordsSection").classList.remove("active");
...
def webhook():
    ...
    if "message" in data:
        ...
        if text == "/start":
            ...
            channel(chat_id)
        elif text == "🎼 سرودها و کتابخانه" or text == "🎼 سرودها":
            songs_menu(chat_id)
        elif text == "📚 کتابخانه":
            library(chat_id)
