...
def sheet(name):
    now = time.time()
    if name in CACHE and now - CACHE[name]["time"] < CACHE_TIME:
...
def search_song(chat_id, text):
    ...
    if partial:
        buttons = [
            [{"text": "🎵 " + value(s, "اسم سرود"), "callback_data": f"songsearch|{i}"}]
            for i, s in enumerate(partial[:10])
        ]
...
def random_prayer(chat_id):
    rows = [r for r in sheet("Prayers") if value(r, "متن دعا")]
    if not rows:
...
def webhook():
    ...
    if "callback_query" in data:
        ...
        elif cb.startswith("songsearch|"):
            index = int(cb.split("|")[1])
            songs = [r for r in sheet("Songs") if value(r, "اسم سرود") and value(r, "فایل")]
            s = songs[index]
            send_audio(chat_id, value(s, "فایل"), "🎶 این سرود تقدیم به شما\n\n🎵 " + value(s, "اسم سرود"))
