...
def random_prayer(chat_id):
    ...
    send_msg(chat_id,
             f"🙏 یک درخواست دعا:\n\n✨ خداوند آرامی و حضور خود را جاری کند\n\n🙏 {count} نفر برای این درخواست دعا کردند",
             {"inline_keyboard": [
                 [{"text": "🙌 من هم دعا می‌کنم", "callback_data": f"praydone|{row_number}"}],
...
def webhook():
    ...
    if "callback_query" in data:
        ...
        if cb.startswith("book|"):
            ...
            if count:
                send_msg(chat_id, f"🙏 ممنون از اینکه در دعا همراه شدید.\n\n🙏 {count} نفر برای این درخواست دعا کردند")
            else:
